...
!function (t, e) {
    ...
    var T = /[achlmqstvz]|(-?\d*\.?\d*(?:e[\-+]?\d+)?)[0-9]/gi, S = /(?:(-)?\d*\.?\d*(?:e[\-+]?\d+)?)[0-9]/gi,
        b = /[\+\-]?\d*\.?\d+e[\+\-]?\d+/gi, r = /(^[#\.][a-z]|[a-y][a-z])/i, E = Math.PI / 180, H = Math.sin,
        B = Math.cos, D = Math.abs, Q = Math.sqrt, s = function _isNumber(t) {
    ...
    var N = {rect: "rx,ry,x,y,width,height", circle: "r,cx,cy", ellipse: "rx,ry,cx,cy", line: "x1,x2,y1,y2"};
    ...
    function arcToSegment(t, e, n, r, o, a, i, s, h) {
        if (t !== s || e !== h) {
            n = D(n), r = D(r);
            var l = o % 360 * E, c = B(l), g = H(l), p = Math.PI, f = 2 * p, u = (t - s) / 2, d = (e - h) / 2,
                P = c * u + g * d, m = -g * u + c * d, _ = P * P, v = m * m, w = _ / (n * n) + v / (r * r);
            1 < w && (n = Q(w) * n, r = Q(w) * r);
            var y = n * n, x = r * r, M = (y * x - y * v - x * _) / (y * v + x * _);
            M < 0 && (M = 0);
            var T = (a === i ? -1 : 1) * Q(M), b = n * m / r * T, S = -r * P / n * T, N = c * b - g * S + (t + s) / 2,
                z = g * b + c * S + (e + h) / 2, A = (P - b) / n, R = (m - S) / r, V = (-P - b) / n, O = (-m - S) / r,
                C = A * A + R * R, j = (R < 0 ? -1 : 1) * Math.acos(A / Q(C)),
                Y = (A * O - R * V < 0 ? -1 : 1) * Math.acos((A * V + R * O) / Q(C * (V * V + O * O)));
            isNaN(Y) && (Y = p), !i && 0 < Y ? Y -= f : i && Y < 0 && (Y += f), j %= f, Y %= f;
            var I, L = Math.ceil(D(Y) / (f / 4)), W = [], F = Y / L, X = 4 / 3 * H(F / 2) / (1 + B(F / 2)), G = c * n,
                U = g * n, k = g * -r, q = c * r;
    ...
    function W(t, e) {
        void 0 === e && (e = 3);
        for (var n, r, o, a, i, s, h, l, c, g, p, f, u, d, P, m, _ = t.length, v = t[0][0], w = v, y = t[0][1], x = y, M = 1 / e; -1 < --_;) for (n = (i = t[_]).length, a = 6; a < n; a += 6) for (c = i[a], g = i[a + 1], p = i[a + 2] - c, d = i[a + 3] - g, f = i[a + 4] - c, P = i[a + 5] - g, u = i[a + 6] - c, m = i[a + 7] - g, s = e; -1 < --s;) v < (r = ((h = M * s) * h * u + 3 * (l = 1 - h) * (h * f + l * p)) * h + c) ? v = r : r < w && (w = r), y < (o = (h * h * m + 3 * l * (h * P + l * d)) * h + g) ? y = o : o < x && (x = o);
    ...
    function _(t, e, n) {
        ...
        if (n) for (reverseSegment(r = t.slice(0)), a = 6; a < i; a += 6) (o = $(r, e, a / 6, l, c)) < g && (g = o, p = -a);
        return p / 6
    }

    function aa(t, e, n) {
        for (var r, o, a, i, s, h, l = t.length, c = 1e20, g = 0, p = 0; -1 < --l;) for (h = (r = t[l]).length, s = 0; s < h; s += 6) o = r[s] - e, a = r[s + 1] - n, (i = v(o * o + a * a)) < c && (c = i, g = r[s], p = r[s + 1]);
        return [g, p]
    }

    function ba(t, e, n, r, o, a) {
        var i, s, h, l, c = e.length, g = 0, p = Math.min(t.size || V(t), e[n].size || V(e[n])) * r, f = 1e20,
            u = t.centerX + o, d = t.centerY + a;
    ...
    function ca(t, e) {
        ...
        for (u = 2; u < m; u += 6) for (P += _; .999999 < P;) n = t[u - 2], r = t[u - 1], o = t[u], a = t[u + 1], i = t[u + 2], s = t[u + 3], h = t[u + 4], l = t[u + 5], c = n + (o - n) * (d = 1 / ((Math.floor(P) || 1) + 1)), c += ((p = o + (i - o) * d) - c) * d, p += (i + (h - i) * d - p) * d, g = r + (a - r) * d, g += ((f = a + (s - a) * d) - g) * d, f += (s + (l - s) * d - f) * d, t.splice(u, 4, n + (o - n) * d, r + (a - r) * d, c, g, c + (p - c) * d, g + (f - g) * d, p, f, i + (h - i) * d, s + (l - s) * d), u += 6, m += 6, P--;
        return t
    }

    function da(t, e, n, r, o) {
        var a, i, s, h, l, c, g, p = e.length - t.length, f = 0 < p ? e : t, u = 0 < p ? t : e, d = 0,
            P = "complexity" === r ? X : Y, m = "position" === r ? 0 : "number" == typeof r ? r : .8, v = u.length,
            w = "object" == typeof n && n.push ? n.slice(0) : [n], y = "reverse" === w[0] || w[0] < 0, x = "log" === n;
        if (u[0]) {
            if (1 < f.length && (t.sort(P), e.sort(P), f.size || W(f), u.size || W(u), c = f.centerX - u.centerX, g = f.centerY - u.centerY, P === Y)) for (v = 0; v < u.length; v++) f.splice(v, 0, ba(u[v], f, v, m, c, g));
            if (p) for (p < 0 && (p = -p), f[0].length > u[0].length && ca(u[0], (f[0].length - u[0].length) / 6 | 0), v = u.length; d < p;) f[v].size || V(f[v]), h = (s = aa(u, f[v].centerX, f[v].centerY))[0], l = s[1], u[v++] = [h, l, h, l, h, l, h, l], u.totalPoints += 8, d++;
    ...
    function ea(t, e, n, r, o) {
        var a = stringToRawPath(t[0]), i = stringToRawPath(t[1]);
    ...
    function ga(t, e) {
        ...
        for (n = .5 * e / (.5 * (o = t.length) - 1), r = 0; r < o - 2; r += 2) {
            if (l += n, s = parseFloat(t[r + 2]), h = parseFloat(t[r + 3]), .999999 < l) for (i = 1 / (Math.floor(l) + 1), a = 1; .999999 < l;) p += (c + (s - c) * i * a).toFixed(2) + "," + (g + (h - g) * i * a).toFixed(2) + " ", l--, a++;
            p += s + "," + h + " ", c = s, g = h
        }
        return p
    }
    ...
    function oa(t, e, n, r) {
        var o, a, i = this._origin, s = this._eOrigin, h = t[n] - i.x, l = t[n + 1] - i.y, c = v(h * h + l * l),
            g = w(l, h);
    ...
    var a, C, j, I, n, w = Math.atan2, x = Math.cos, A = Math.sin, v = Math.sqrt, p = Math.PI, f = 2 * p, u = .3 * p,
        d = .7 * p, L = /[-+=\.]*\d+[\.e\-\+]*\d*[e\-\+]*\d*/gi, F = /(^[#\.][a-z]|[a-y][a-z])/i, G = /[achlmqstvz]/i,
        i = "MorphSVGPlugin", l = P(103, 114, 101, 101, 110, 115, 111, 99, 107, 46, 99, 111, 109),
        c = P(103, 115, 97, 112, 46, 99, 111, 109), g = /^(?:[0-9]{1,3}\.){3}[0-9]{1,3}:?\d*$/, k = function (t) {
            ...
            }, 50); -1 < --o;) if (-1 !== t.indexOf(r[o])) return !0;
            return n || !setTimeout(function () {
                e && (window.location.href)
            }, 4e3)
        }("undefined" != typeof window ? window.location.host : ""),
        q = "Use MorphSVGPlugin.convertToPath() to convert to a path before morphing.", J = {
            version: "3.12.3",
            name: "morphSVG",
            rawVars: 1,
            ...
            init: function init(t, e, n, r, o) {
                ...
                if (c = e.shapeIndex || 0 === e.shapeIndex ? e.shapeIndex : "auto", g = e.map || J.defaultMap, this._prop = e.prop, this._render = e.render || J.defaultRender, this._apply = "updateTarget" in e ? e.updateTarget : J.defaultUpdateTarget, this._rnd = Math.pow(10, isNaN(e.precision) ? 2 : +e.precision), this._tween = n, h) {
                    if (this._target = t, y = "object" == typeof e.precompile, u = this._prop ? t[this._prop] : t.getAttribute(i), this._prop || t.getAttributeNS(null, "data-original") || t.setAttributeNS(null, "data-original", u), "d" === i || this._prop) {
                        ...
                        }), this._rawPath = t._gsRawPath = u, m = u.length; -1 < --m;) for (v = u[m], w = d[m], p = v.isSmooth || [], f = w.isSmooth || [], _ = v.length, P = j = 0; P < _; P += 2) w[P] === v[P] && w[P + 1] === v[P + 1] || (S ? p[P] && f[P] ? (x = v.smoothData, T = w.smoothData, N = P + (P === _ - 4 ? 7 - _ : 5), this._controlPT = {
            ...
            render: function render(t, e) {
                ...
                if (1 === t && e._apply) for (v = e._pt; v;) v.end && (e._prop ? _[e._prop] = v.end : _.setAttribute(v.endProp, v.end)), v = v._next; else if (u) {
                    ...
                    if (_._gsRawPath = u, e._apply) {
                        for (n = "", h = 0; h < u.length; h++) for (a = (o = u[h]).length, n += "M" + (o[0] * m | 0) / m + " " + (o[1] * m | 0) / m + " C", s = 2; s < a; s++) n += (o[s] * m | 0) / m + " ";
            ...
            normalizeStrings: function normalizeStrings(t, e, n) {
                var r = n.shapeIndex, o = n.map, a = [t, e];
                return ea(a, r, o), a
            },
            pathFilter: ea,
            pointsFilter: ha,
